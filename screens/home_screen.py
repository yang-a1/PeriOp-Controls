...
class HomeScreen(tk.Frame):
    ...
    def init_status_panel(self):
        ...
        self.status = tk.Label(self, textvariable=self.status_var, font=("Inter", 14), bg="#F3F6FB", fg="#1E293B")
        self.status.place(x=40, y=360)
    def update_status(self, msg: str):
        """Update the status message shown in the status label."""
    ...
    def update_temperature_display(self):
        ...
        # Try to extract the numeric temperature value
        try:
            temp_value = float(temp_string.split()[0])  # Get the numeric part before the space
        ...
        if temp_value is None:
            temp_chars = ['-', '-', '.', '-', 'C']
            self.update_status("Invalid temperature reading")

        else:
            # Evaluate temperature range and update status accordingly
            if temp_value < 36.0:
                self.update_status("⚠️ Too Low")
            elif temp_value > 38.0:
                self.update_status("🔥 Too High")
            else:
        ...
        if not hasattr(self, 'temp_char_list'):
            ...
            if len(temp_chars) > len(self.temp_char_list):
                temp_chars = ['-', '-', '.', '-', 'C']
                self._render_temp_chars(temp_chars, digits)
                self.update_status("Invalid temperature reading detected")
